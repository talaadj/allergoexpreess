...
export default async function handler(request: Request) {
    ...
    try {
        ...
        const { orderId, patientName, phone, date, birthDate, medications } = data;
    ...
    `;
        ...
        // Add birth_date column if it doesn't exist (safe migration)
        try {
            await sql`ALTER TABLE results ADD COLUMN IF NOT EXISTS birth_date VARCHAR(255);`;
        } catch (e) {
            // Column might already exist, ignore error
            console.log('birth_date column already exists or error adding it:', e);
        }

        // Insert data
        await sql`
      INSERT INTO results (order_id, patient_name, phone, date, birth_date, medications)
      VALUES (${orderId}, ${patientName}, ${phone || null}, ${date}, ${birthDate || null}, ${JSON.stringify(medications)})
